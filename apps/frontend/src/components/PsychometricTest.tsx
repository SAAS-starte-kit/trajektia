...
function ResultsPhase({
  ...
  // ── Feature Flags (réactifs aux événements trajektia:feature-flag-change)
  const [showFacetMirror, setShowFacetMirror] = useState(false);
  const [showStrainIndex, setShowStrainIndex] = useState(false);

  useEffect(() => {
    const syncFlags = () => {
      setShowFacetMirror(isFeatureEnabled("FF_PSYCHO_FACET_MIRROR"));
      setShowStrainIndex(isFeatureEnabled("FF_TAT_STRAIN_INDEX"));
    };
    syncFlags();
    window.addEventListener("trajektia:feature-flag-change", syncFlags);
    return () => window.removeEventListener("trajektia:feature-flag-change", syncFlags);
  }, []);

  // ── Calcul des sous-facettes du Névrosisme (approximation IPIP-50)
  const nevrosisme = 100 - results.bigFive.Stabilite_Emotionnelle;
  const facettes = [
    {
      nom: "Vulnérabilité",
      description: "Sensibilité au stress et aux situations de pression extrême.",
      score: Math.min(100, Math.round(nevrosisme * 0.38)),
      couleur: "#EF4444",
    },
    {
      nom: "Anxiété",
      description: "Tendance à anticiper les risques et à ruminer les erreurs passées.",
      score: Math.min(100, Math.round(nevrosisme * 0.35)),
      couleur: "#F97316",
    },
    {
      nom: "Volatilité émotionnelle",
      description: "Amplitude et vitesse des fluctuations émotionnelles au quotidien.",
      score: Math.min(100, Math.round(nevrosisme * 0.27)),
      couleur: "#8B5CF6",
    },
  ];

  // ── Interactions TAT (contextes O*NET approximés depuis OnetWorkStyles)
  const estimatedOnetContexts: OnetContextScores = {
    PRESSION_TEMPORELLE: careerMatches[0]
      ? (() => {
          const ws = (careerMatches[0] as any).onet_work_styles as Array<{id:string; score:number}> | undefined;
          return ws?.find(s => s.id === "4.C.3.d.1")?.score ?? 55;
        })()
      : 55,
    FREQUENCE_CONFLITS: 40,
    CONSEQUENCES_ERREUR: 50,
    TRAVAIL_STRUCTURE: careerMatches[0]
      ? (() => {
          const ws = (careerMatches[0] as any).onet_work_styles as Array<{id:string; score:number}> | undefined;
          return ws?.find(s => s.id === "4.C.3.b.8a")?.score ?? 60;
        })()
      : 60,
  };

  const tatInteractions = calculateTATInteractions(
    { nevrosisme, stabiliteEmotionnelle: results.bigFive.Stabilite_Emotionnelle },
    estimatedOnetContexts
  );
  ...
  return (
    ...
    >
      ...
      {/* ─── FF_PSYCHO_FACET_MIRROR : Miroir des Sous-Facettes ─── */}
      {showFacetMirror && (
        <div className="bg-white/60 dark:bg-slate-800/50 backdrop-blur-sm border border-violet-200 dark:border-violet-700/50 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xl">🔬</span>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
              Miroir des Facettes — Stabilité Émotionnelle
            </h2>
            <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300 border border-violet-200 dark:border-violet-700">
              IPIP-50
            </span>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">
            Décomposition de votre axe de Stabilité Émotionnelle en trois sous-facettes IPIP.
            Ces nuances permettent un accompagnement c.o. plus ciblé.
          </p>
            ...
            {facettes.map((f) => (
              <div key={f.nom}>
                <div className="flex justify-between items-center mb-1">
                  <div>
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-100">{f.nom}</span>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{f.description}</p>
                  </div>
                  <span className="text-sm font-bold tabular-nums text-slate-900 dark:text-white ml-4 flex-shrink-0">
                    {formatPomp(f.score)}
                  </span>
                </div>
          ...
          <p className="text-[11px] text-slate-400 dark:text-slate-600 mt-4 italic">
            Scores approximés depuis l'item Stabilité Émotionnelle IPIP-50. Pondérations :
            Vulnérabilité ×0.38, Anxiété ×0.35, Volatilité ×0.27.
          </p>
        </div>
      )}

      {/* ─── FF_TAT_STRAIN_INDEX : Indice d'Écart de Tension ─── */}
      {showStrainIndex && (
        <div className="bg-white/60 dark:bg-slate-800/50 backdrop-blur-sm border border-slate-200 dark:border-slate-700 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xl">⚖️</span>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
              Indice d'Écart de Tension (Strain Index)
            </h2>
            <span className="ml-auto text-[10px] font-bold px-2 py-0.5 rounded bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-700">
              TAT
            </span>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-1 leading-relaxed">
            Formule : <code className="font-mono bg-slate-100 dark:bg-slate-800 px-1 rounded">Strain = Yc − Xs</code> — Exigence contextuelle O*NET moins résistance IPIP.
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mb-5">
            Réf. : Tett &amp; Burnett (2003) · Contextes estimés pour votre métier top-1 compatible.
          </p>

          <div className="space-y-3">
            {tatInteractions.map((interaction: TATInteraction) => {
              const style = getStrainZoneStyle(interaction.strainResult.zone);
              const gapAbs = Math.abs(interaction.strainResult.gap);
              const gapPct = Math.min(100, Math.round(gapAbs));
              return (
                <div
                  key={interaction.contextKey}
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className={`text-xs font-bold ${style.text}`}>
                          {interaction.contextLabel}
                        </span>
                        <code className="text-[10px] text-slate-400 font-mono bg-slate-100 dark:bg-slate-800 px-1 rounded">
                          {interaction.onetCode}
                        </code>
                      </div>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">
                        Trait activé : <em>{interaction.traitActivated}</em>
                      </p>
                    </div>
                    <span className={`flex-shrink-0 text-[10px] font-bold px-2 py-1 rounded-lg border ${style.badge}`}>
                      {interaction.strainResult.zone === "Verte" ? "✓" : interaction.strainResult.zone === "Rouge" ? "⚠" : "○"}{" "}
                      {interaction.strainResult.zone}
                    </span>
                  </div>
                  ...
                  <div className="mt-3">
                    <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                      <span>Écart Yc−Xs : {interaction.strainResult.gap > 0 ? "+" : ""}{Math.round(interaction.strainResult.gap)} pts</span>
                      <span>{interaction.strainResult.label.split(" — ")[1]}</span>
                    </div>
                    <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        style={{
                          backgroundColor:
                            interaction.strainResult.zone === "Rouge" ? "#EF4444"
                            : interaction.strainResult.zone === "Orange" ? "#F97316"
                            : "#10B981",
                        }}
                        initial={{ width: 0 }}
                  ...
                  <details className="mt-2">
                    <summary className={`text-[11px] font-medium cursor-pointer ${style.text} hover:opacity-80`}>
                      Analyse clinique &amp; recommandation
                    </summary>
                    <div className="mt-2 space-y-1.5">
                      <p className="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
                        {interaction.strainResult.description}
                      </p>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 italic leading-relaxed">
                        💡 {interaction.strainResult.recommendation}
                      </p>
                    </div>
                  </details>
                </div>
          ...
          <p className="text-[11px] text-slate-400 dark:text-slate-600 mt-4 italic">
            Résistance personnelle = Stabilité Émotionnelle IPIP = {formatPomp(results.bigFive.Stabilite_Emotionnelle)}.
            Contextes O*NET estimés pour une analyse personnalisée, consultez un c.o. OCCOQ.
          </p>
        </div>
      )}
