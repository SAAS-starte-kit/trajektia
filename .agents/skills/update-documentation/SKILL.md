...
Ce skill définit la procédure standard pour maintenir à jour la documentation principale du projet Trajektia, située dans le dossier `packages/ckg/`.
...
- `packages/ckg/MANUEL_PSYCHOMETRIQUE.md` : Pour les aspects liés aux modèles psychologiques (RIASEC, Big Five, etc.).
- `packages/ckg/MANUEL_METHODOLOGIQUE_CKG.md` : Pour la structure du graphe de connaissances, les relations et l'ingestion des données.
- `packages/ckg/PLAN_ACTION.md` : Pour suivre l'avancement des tâches et les prochaines étapes.
- `packages/ckg/LIVRE_BLANC_TRAJEKTIA.md` : Pour la vision d'ensemble du produit.
...
2. **Recherche et Localisation :**
   - Lisez le contenu actuel du ou des fichiers concernés dans le dossier `packages/ckg/` avec l'outil `view_file`.
